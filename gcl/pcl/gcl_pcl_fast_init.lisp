...
(defun make-instance-1 (class &rest initargs)
  ...
  (let* ((key (initialize-info-key info))
    ...
    (ecase name
	 ...
	 (setf (initialize-info-cached-new-keys info) new-keys)))
      ((combined-initargs-form-list)
       (multiple-value-bind (initargs-form-list new-keys)
	   (make-default-initargs-form-list class keys nil)
	 (setf (initialize-info-cached-combined-initargs-form-list info) 
	       initargs-form-list)
	 (setf (initialize-info-cached-new-keys info) new-keys)))
      ((default-initargs-function)
	 ...
	 (setf (initialize-info-cached-shared-initialize-nil-function info)
      ...
      ((constants combined-initialize-function)
       (let ((initargs-form-list (initialize-info-combined-initargs-form-list info))
...
(defun get-make-instance-function-internal (class key initargs 
  ...
  (let* (#|(class-key (car key))|#
    ...
    (cond ((or (cdr allocate-instance-methods)
	   ...
	   (make-instance-function-complex
	    key class keys
	    initialize-instance-methods shared-initialize-methods))
	  (t #|(or (not (standard-class-p class)) (not (symbolp class-key))
	       initialize-instance-methods shared-initialize-methods)|#
	   (make-instance-function-simple
	    key class keys
	    initialize-instance-methods shared-initialize-methods))
	  #|(t
	   (make-instance-function-basic
	    key class keys))|#)))
...
(defmacro copy-slots (slots-init)
  #-(or lucid cmu17)
  `(copy-seq ,slots-init)
  #+(or lucid cmu17)
  `(let* ((init ,slots-init)
	  (len (length init))
	  (v #+lucid (system:new-simple-vector len)
	     #+cmu17 (lisp::allocate-vector #.vm:simple-vector-type len len)))
     (declare (simple-vector init v)
	      (type #-cmu fixnum #+cmu lisp::index len))
     (dotimes (i len v)
       (declare (type #-cmu fixnum #+cmu lisp::index i))
       (setf (svref v i) (svref init i)))))

(defmacro allocate-standard-instance--macro (wrapper slots-init)
  #-new-kcl-wrapper
  `(let ((instance (%%allocate-instance--class)))
     (setf (std-instance-wrapper instance) ,wrapper)
     (setf (std-instance-slots instance) (copy-slots ,slots-init))
     instance)
  #+new-kcl-wrapper
  `(allocate-standard-instance ,wrapper ,slots-init))

(defmacro with-make-instance-function-valid-p-check (initargs-form &body body)
  `(let ((current-class (if class-cell
			    (find-class-from-cell class-key class-cell)
			    class-symbol)))
     (if (or (not (eq current-class class-symbol))
	     (invalid-wrapper-p wrapper))
	 (make-instance-function-trap current-class ,initargs-form)
	 (progn ,@body))))

(defun make-instance-function-trap (class-symbol initargs)
  (let* ((info (initialize-info class-symbol initargs))
	 (fn (initialize-info-make-instance-function info)))
    (declare (type function fn))
    (funcall fn class-symbol initargs)))

(defun make-instance-function-simple (key class keys
					  initialize-instance-methods 
					  shared-initialize-methods)
  (let* ((class-key (car key))
	 (class-cell (when (symbolp class-key)
		       (find-class-cell class-key nil)))
	 ...
	 (allocate-macro
	  (cond ((standard-class-p class)
		 'allocate-standard-instance--macro)))
	...
	(get-simple-initialization-function class keys (caddr key))
      (if (eq allocate-macro 'allocate-standard-instance--macro)
	  #'(lambda (class-symbol &rest initargs) 
	      (with-make-instance-function-valid-p-check initargs
		(let ((instance 
		       (allocate-standard-instance--macro wrapper constants)))
		  (call-initialize-function initialize-function instance initargs)
...
(defun make-instance-function-basic (key class keys)
  (let* ((class-key (car key))
	 (class-cell (find-class-cell class-key nil))
	 (wrapper (class-wrapper class)))
    (multiple-value-bind (initialize-function constants)
	(get-simple-initialization-function class keys (caddr key))
      #'(lambda (class-symbol &rest initargs)
	  (let ((current-class (find-class-from-cell class-key class-cell)))
	    (if (or (not (eq current-class class-symbol))
		    (invalid-wrapper-p wrapper))
		(make-instance-function-trap current-class initargs-form)
		(let ((instance 
	  ...
	  (let* ((initform (slot-definition-initform slot))
		 ...
		 (forms (cond ((null initfunction)
				 ...
				 (if (and simple-p (integerp location))
				     (progn (setf (svref constants location) value)
					    nil)
				     `((const ,value)
				       (instance-set ,pv-offset ,slot)))))
			      (t
...
(defmacro iis-body (&body forms)
  (let ((vars '(initargs-tail pv slots wrapper class value)))
    `(let ((initargs-tail initargs)
	   (pv (car pv-cell))
	   (slots nil)
	   (wrapper #+cmu17 (kernel:layout-of instance) #-cmu17 nil)
	   class
	   value)
       ,@(progn
	  #-cmu vars
	  #+cmu `((declare (ignorable ,@vars))))
       #+cmu17
       (cond ((not (typep wrapper 'wrapper)))
       ...
       (cond ((std-instance-p instance)
	      (setq slots (std-instance-slots instance))
	      (setq wrapper (std-instance-wrapper instance)))
	     ((fsc-instance-p instance)
	      (setq slots (fsc-instance-slots instance))
	      (setq wrapper (fsc-instance-wrapper instance)))
	     (t
	      (setq wrapper (wrapper-of instance))))
