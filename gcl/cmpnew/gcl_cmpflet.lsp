	  ...
	  (cond ((eq c1 t) 
		...
		((let* ((c1 (or c1 (set fun t)))
		   ...
		   (cond ((iflag-p (info-flags (cadr res)) provisional)
			  (keyed-cmpnote (list (fun-name fun) 'provisional) "local fun ~s provisionally processed" (fun-name fun))
			  (set fun nil)
			  res);FIXME think about this
			 ((setq tmp (calls-blocked-fun-p fun i))
